...
class ConfusionMatrix(object):
    ...
    def update(self, a, b):
        ...
        if self.mat is None:
            # 创建混淆矩阵
            self.mat = torch.zeros((n, n), dtype=torch.int64, device=b.device)
        with torch.no_grad():
            # 寻找GT中为目标的像素索引
            k = (a >= 0) & (a < n)
            # 统计像素真实类别a[k]被预测成类别b[k]的个数(这里的做法很巧妙)
            inds = n * a[k].to(torch.int64).to('cuda') + b[k].to('cuda')
            inds = inds.to('cuda')
    ...
    def compute(self):
        ...
        acc_global =( h[0][1]+h[1][0]) / h.sum()
        #0背景 1前景 第一行.sum为背景数量 第二行.sum为前景数量 正前景 负背景
        #【   fp     tn
        #     tp     fn
        # 】
        #0代表背景 1代表前景
        ...
        return acc_global,se*100,sp*100,f1*100
    ...
    def back(self):
        acc_global, se, sp,f1 = self.compute()
        my_str = ('acc: {:.4f}\n'
                  'se:{:.4f}\n'
                  'sp:{:.4f}\n'
                  'F1:{:.4f}'
            ...
            sp,
        f1)
        return my_str, se, sp, acc_global,f1
    # def __str__(self):
    #     acc_global, se, sp = self.compute()
    #     my_str=('预测正确的样本百分比acc: {:.4f}\n'
    #         'se:{:.4f}\n'
    #         'sp:{:.4f}'
    #         ).format(
    #         acc_global.item() * 100,
    #         se,
    #         sp)
    #     return my_str,se,sp,acc_global
...
def dice_coeff(x: torch.Tensor, target: torch.Tensor, ignore_index: int = -100, epsilon=1e-6):
    # Average of Dice coefficient for all batches, or for a single_gaosi_guangzhao mask
