...
def test(model, dataloader, args, dice_criterion,epoch=0):
    ...
    with torch.no_grad():
        ...
        for i, data in bar:
            ...
            if True:
                predict_epoch = os.path.join(args.snapshot_dir, str(epoch))
                ...
                for j in range(output.shape[0]):
                    ...
                    output_tensor = gt + torch.from_numpy(2* (seg_result/255))
                    assert output_tensor.shape[0]==1
                    output_img = Image.fromarray(VESSEL_PALETTE[output_tensor.squeeze().squeeze().numpy().astype(int)])
                    output_img.save(os.path.join(predict_epoch,'color'+data['name'][j].split('\\')[-1]))
            ...
            dice_loss += dice_criterion(output_,gt,weight=np.array(-1))
    ...
    val_info, se, sp, acc, F1 = confmat.back()
    val_info += f'\nAUC:{Auc:.4f}\n' + 'val_dice:{:.4f}\n'.format(1 - dice_loss)
    print(val_info)
